...
describe('ingestion: owners', function() {
  ...
  describe('.exists()', function() {
    ...
    it('should check if multiple owner exist if an array of usernames is passed', (done) => {
      client.owners
        .exists(['fake1', 'fake2', 'owner1', 'fake4'])
        .then((response) => {
          expect(response).toEqual([{ fake1: false }, { fake2: false }, { owner1: true }, { fake4: false }]);
          done();
        })
        .catch((error) => {
          fail(error);
